(() => {
  ...
  /* =====================================================
     PATCH LEVEL 8 — PANJI MOOD BOOSTER
     Catatan: patch ini sengaja tidak mengubah tampilan awal lama.
     Bonus hanya muncul ketika pemain sampai Level 8.
     ===================================================== */

  let bonusRunFrame = null;
  let bonusRunKeyHandler = null;
  let bonusRunSlideTimer = null;

  const BONUS_LEVEL_INDEX = 7;
  const BONUS_RUN_DURATION = 45;
  const BONUS_INVINCIBLE_MS = 2200;
  const BONUS_SLIDE_MS = 650;

  const BONUS_RUN_CHALLENGE = {
    type: 'bonusRun',
    title: 'Soal 8 — PANJI Mood Booster',
    caseTitle: 'Bonus Santuy — Dunia Mimpi PANJI',
    desc: 'Bonus time! Otak kita sudah kerja keras. Sekarang bantu PANJI lari santai, ambil item semangat, hindari jebakan lucu, dan balikin mood sebelum lanjut ke level berikutnya.',
    budget: 'Bonus Santuy',
    difficulty: 'Level 8 - Bonus',
    timeLimit: BONUS_RUN_DURATION,
  ...
  const BONUS_COLLECTIBLES = [
    { key: 'star', label: 'Bintang Semangat', icon: '⭐', points: 4, y: [112, 210], useLogo: true, lines: ['Bintang semangat masuk!', 'Yes, semangat naik lagi!'] },
    { key: 'coin', label: 'Koin PANJI', icon: '🪙', points: 4, y: [84, 190], useLogo: true, lines: ['Koin PANJI aman!', 'Sip, koin semangat masuk!'] },
    { key: 'coffee', label: 'Kopi Pagi', icon: '☕', points: 5, y: [42, 115], lines: ['Wah, nyawa balik lagi!', 'Kopi masuk, fokus balik!'] },
    { key: 'mie', label: 'Mie Rebus Lembur', icon: '🍜', points: 5, y: [48, 122], lines: ['Mode lembur aktif!', 'Mie rebus siap, gas terus!'] },
    { key: 'doc', label: 'Dokumen Aman', icon: '📄', points: 4, y: [80, 175], lines: ['Dokumen aman, hati tenang!', 'Arsip rapi, mood happy!'] },
    { key: 'gift', label: 'Bonus Poin', icon: '🎁', points: 6, y: [100, 205], lines: ['Yeay, bonus poin!', 'Hadiah masuk, lanjut lari!'] }
  ];

  const BONUS_HAZARDS = [
    { key: 'sleep', label: 'Ngantuk', icon: '💤', penalty: 3, risk: 1, y: [34, 80], line: 'Aduh, ngantuk menyerang!' },
    { key: 'berkas', label: 'Berkas Numpuk', icon: '📚', penalty: 4, risk: 1, y: [36, 88], line: 'Aduh, ketiban administrasi!' },
    { key: 'deadline', label: 'Deadline', icon: '🔥', penalty: 5, risk: 2, y: [36, 90], line: 'Deadline menyerang!' },
    { key: 'revisi', label: 'Revisi Dadakan', icon: '😵', penalty: 4, risk: 1, y: [36, 92], line: 'Yah, revisi dadakan datang lagi!' },
    { key: 'loading', label: 'Loading Lama', icon: '🐌', penalty: 3, risk: 1, y: [34, 84], line: 'Sabar... sistem sedang kontemplasi.' }
  ];

  function ensureBonusLevel8Inserted() {
    if (CHALLENGES.some(item => item && item.type === 'bonusRun')) return;
    const bonus = { ...BONUS_RUN_CHALLENGE };
    if (CHALLENGES.length > BONUS_LEVEL_INDEX) CHALLENGES.splice(BONUS_LEVEL_INDEX, 0, bonus);
    else CHALLENGES.push(bonus);
  }
  ...
  function clearBonusRunLoop() {
    ...
    if (bonusRunSlideTimer) {
      clearTimeout(bonusRunSlideTimer);
      bonusRunSlideTimer = null;
    }
  }
  ...
  function createBonusRunState(challenge) {
    return {
      ...
      startedAt: 0,
      lastFrameAt: 0,
      timeLeft: Number(challenge.timeLimit || BONUS_RUN_DURATION),
      runnerY: 0,
      velocity: 0,
      onGround: true,
      jumpsLeft: 2,
      ...
      hit: 0,
      combo: 0,
      lastStatus: 'Tekan Spasi/Tap untuk lompat. Bisa double jump. Tekan S / Panah bawah untuk sliding.',
  ...
  function setBonusStatus(text, mood) {
    const run = getBonusRunState();
    if (run) run.lastStatus = text;
    if (panjiUserMinimized) {
      setPanjiMoodOnly(mood || 'thinking');
      return;
    }
    showPanji(text, mood || 'thinking');
  }
  ...
  function startBonusRun() {
    ...
    run.timeLeft = Number(challenge.timeLimit || BONUS_RUN_DURATION);
    run.nextSpawnAt = .65;
    GAME_STATE.progress = 0;

    enableBonusControls();
    setBonusStatus('Bonus time! Bantu aku ngumpulin energi. Spasi/Tap untuk lompat, bisa double jump, dan tekan S / Panah bawah untuk sliding.', 'happy');
    renderGame();
    updateBonusUi();
    bonusRunFrame = requestAnimationFrame(stepBonusRun);
  }

  function enableBonusControls() {
    clearBonusRunLoop();
  ...
  function bonusJump() {
    ...
    if (run.jumpsLeft <= 0) return;

    run.isSliding = false;
    run.slideUntil = 0;
    run.onGround = false;
    run.velocity = run.jumpsLeft === 2 ? 720 : 610;
    run.jumpsLeft -= 1;
  ...
  function bonusSlide() {
    ...
    run.isSliding = true;
    run.slideUntil = performance.now() + BONUS_SLIDE_MS;
    run.lastStatus = 'PANJI sliding! Hindari rintangan rendah, gaya dulu dikit.';
    if (bonusRunSlideTimer) clearTimeout(bonusRunSlideTimer);
    bonusRunSlideTimer = setTimeout(() => {
      const current = getBonusRunState();
      if (current) current.isSliding = false;
      updateBonusUi();
    }, BONUS_SLIDE_MS + 40);
    updateBonusUi();
  }

  function pickBonusObject(elapsed) {
    const hazardChance = elapsed > 30 ? .50 : elapsed > 18 ? .42 : .32;
    const hazard = Math.random() < hazardChance;
    const meta = hazard
      ? BONUS_HAZARDS[Math.floor(Math.random() * BONUS_HAZARDS.length)]
      : BONUS_COLLECTIBLES[Math.floor(Math.random() * BONUS_COLLECTIBLES.length)];
    const range = meta.y || [40, 120];
    return {
      id: 'bonus-' + Date.now() + '-' + Math.random().toString(16).slice(2),
      category: hazard ? 'hazard' : 'collectible',
      key: meta.key,
      label: meta.label,
      icon: meta.icon,
      ...
      x: 1040,
      y: range[0] + Math.random() * Math.max(1, range[1] - range[0]),
      width: hazard ? 62 : 56,
      height: hazard ? 62 : 56,
  ...
  function bonusCollides(obj, stageWidth, now) {
    const run = getBonusRunState();
    if (!run) return false;
    if (now < run.invincibleUntil) return false;

    const runnerLeft = Math.max(66, stageWidth * .12);
    const runnerBottom = 34 + Number(run.runnerY || 0);
    const runnerWidth = run.isSliding ? 104 : 92;
    const runnerHeight = run.isSliding ? 56 : 104;
    ...
    return !(objRight < runnerLeft + 8 || objLeft > runnerRight - 8 || objTop < runnerBottom + 8 || objBottom > runnerTop - 8);
  }

  function collectBonus(obj) {
    const run = getBonusRunState();
    ...
    run.runScore += obj.points;
    GAME_STATE.score += obj.points;
    const lines = obj.lines && obj.lines.length ? obj.lines : [obj.label + ' didapat!'];
    setBonusStatus(lines[Math.floor(Math.random() * lines.length)], 'happy');
    const stage = root && root.querySelector('#psBonusStage');
    if (stage) popScore(stage, '+' + obj.points, 'ok');
  }

  function hitBonusHazard(obj, now) {
    const run = getBonusRunState();
    if (!run || obj.hit) return;
    obj.hit = true;
    run.hit += 1;
    run.combo = 0;
    run.counters[obj.key] = Number(run.counters[obj.key] || 0) + 1;
    run.runScore = Math.max(0, run.runScore - obj.penalty);
    GAME_STATE.score = Math.max(0, GAME_STATE.score - obj.penalty);
    GAME_STATE.risk += obj.risk || 1;
    run.invincibleUntil = now + BONUS_INVINCIBLE_MS;
    setBonusStatus((obj.line || obj.label + ' kena!') + ' Aku kebal sebentar, item yang lewat tidak dihitung dulu ya.', 'sad');
    const stage = root && root.querySelector('#psBonusStage');
    if (stage) popScore(stage, '-' + obj.penalty, 'bad');
  }

  function stepBonusRun(now) {
    const challenge = getCurrentChallenge();
    const run = getBonusRunState();
    ...
    const stage = root && root.querySelector('#psBonusStage');
    const stageWidth = Math.max(760, Number(stage && stage.clientWidth || 980));
    const dt = Math.min(.045, Math.max(.012, (now - Number(run.lastFrameAt || now)) / 1000));
    run.lastFrameAt = now;

    const elapsed = Math.max(0, (now - Number(run.startedAt || now)) / 1000);
    run.timeLeft = Math.max(0, Number(challenge.timeLimit || BONUS_RUN_DURATION) - elapsed);
    GAME_STATE.progress = Math.max(0, Math.min(100, Math.round((elapsed / Math.max(1, Number(challenge.timeLimit || BONUS_RUN_DURATION))) * 100)));

    if (run.isSliding && now > run.slideUntil) run.isSliding = false;

    if (!run.onGround) {
      run.velocity -= 1850 * dt;
      run.runnerY = Math.max(0, Number(run.runnerY || 0) + run.velocity * dt);
      if (run.runnerY <= 0) {
        run.runnerY = 0;
        run.velocity = 0;
        run.onGround = true;
    ...
    const speed = elapsed < 15 ? 305 : elapsed < 30 ? 355 : 405;
    run.nextSpawnAt -= dt;
    if (run.nextSpawnAt <= 0) {
      run.objects.push(pickBonusObject(elapsed));
      const minGap = elapsed < 12 ? .88 : elapsed < 28 ? .70 : .56;
      const maxGap = elapsed < 12 ? 1.20 : elapsed < 28 ? .98 : .78;
      run.nextSpawnAt = minGap + Math.random() * Math.max(.10, maxGap - minGap);
    }

    run.objects.forEach(obj => {
      obj.x -= speed * dt;
      obj.pulse += dt * 4;
      if (!obj.hit && bonusCollides(obj, stageWidth, now)) {
        if (obj.category === 'hazard') hitBonusHazard(obj, now);
        else collectBonus(obj);
      }
    });
  ...
  function renderBonusObject(obj) {
    const cls = ['ps-bonus-item', obj.category === 'hazard' ? 'hazard' : 'collectible', obj.useLogo ? 'logo-item' : '', 'item-' + obj.key].join(' ');
    return '<div class="' + cls + '" style="left:' + obj.x + 'px; bottom:' + obj.y + 'px; width:' + obj.width + 'px; height:' + obj.height + 'px; transform:rotate(' + obj.rotate + 'deg) scale(' + (1 + Math.sin(obj.pulse) * .04) + ');"><span>' + escapeHtml(obj.icon) + '</span></div>';
  }
  ...
  function updateBonusUi() {
    ...
    setText('#psTopProgressValue', progress + '%');
    setText('#psTopScoreValue', GAME_STATE.score);
    setText('#psTopRiskValue', GAME_STATE.risk);
    setText('#psTopWrongValue', GAME_STATE.wrong);
    setText('#psCaseScoreValue', GAME_STATE.score);
    setText('#psCaseRiskValue', GAME_STATE.risk);
    setText('#psLevelTimeText', Math.ceil(run.timeLeft) + 's');
    setText('#psBonusTimeValue', Math.ceil(run.timeLeft));
    setText('#psBonusRunScore', run.runScore);
    setText('#psBonusCollectedValue', run.collected);
    setText('#psBonusHitValue', run.hit);
    setText('#psBonusComboValue', 'x' + run.combo);
    setText('#psBonusStatusText', run.lastStatus);
    setText('#psBonusJumpValue', run.onGround ? '2/2' : run.jumpsLeft + '/2');
    setText('#psBonusShieldValue', now < run.invincibleUntil ? (Math.ceil((run.invincibleUntil - now) / 1000) + 's') : '-');

    const mainBar = root.querySelector('#psMainProgressBar');
    if (mainBar) mainBar.style.width = progress + '%';
    const timeBar = root.querySelector('#psLevelTimeBar');
    if (timeBar) timeBar.style.width = Math.max(0, Math.min(100, (run.timeLeft / Math.max(1, Number(challenge.timeLimit || BONUS_RUN_DURATION))) * 100)) + '%';
    ...
    if (objectsEl) objectsEl.innerHTML = run.objects.map(renderBonusObject).join('');

    const runner = root.querySelector('#psBonusRunner');
    if (runner) {
      runner.style.bottom = (34 + Math.max(0, Number(run.runnerY || 0))) + 'px';
      runner.classList.toggle('is-sliding', run.isSliding);
      runner.classList.toggle('is-invincible', now < run.invincibleUntil);
      runner.classList.toggle('is-jumping-state', !run.onGround);
    }
  }
  ...
  function finishBonusRun() {
    ...
    GAME_STATE.score += 20;
    clearBonusRunLoop();
    addLog('ok', 'Bonus level selesai', 'Mood Booster selesai. Skor santuy ' + run.runScore + '. Kopi ' + (run.counters.coffee || 0) + ', bintang ' + (run.counters.star || 0) + ', bonus poin ' + (run.counters.gift || 0) + '.');
    showToast('PANJI Mood Booster selesai. Otomatis lanjut...', 'ok');
    setBonusStatus('Mantap! Mood naik, lanjut beresin paket!', 'happy');
    renderGame();
    updateBonusUi();
      ...
      return `
        <div class="ps-bonus-shell">
          <div class="ps-bonus-intro-card">
            ...
            <p>
              Bonus level ini bukan ujian. PANJI akan lari otomatis ke kanan. Tugas kamu cuma <b>lompat</b>, <b>double jump</b>, dan <b>sliding</b>.
              Ambil item semangat seperti kopi, koin, dan bintang. Hindari deadline, revisi dadakan, dan berkas numpuk.
              Kalau kena perangkap, PANJI jadi <b>invincible beberapa detik</b>, jadi item/rintangan yang lewat tidak dihitung dulu.
            </p>
            <div class="ps-bonus-chip-row">
              <span class="ps-bonus-chip good">⭐/🪙 dari <b>assets/logo-sippbj.png</b></span>
              <span class="ps-bonus-chip good">Spasi/Tap = Lompat</span>
              <span class="ps-bonus-chip good">Double Jump aktif</span>
              <span class="ps-bonus-chip good">S / ↓ = Sliding</span>
            ...
            <div class="ps-bonus-controls-note">
              <div><b>Durasi:</b> ${Number(challenge.timeLimit || BONUS_RUN_DURATION)} detik</div>
        ...
        <div class="ps-bonus-shell">
          <div class="ps-bonus-finish-card">
            ...
            <div class="ps-bonus-summary-grid">
              ...
              <div><label>Item Terkumpul</label><strong>${run.collected}</strong></div>
              <div><label>Kena Jebakan</label><strong>${run.hit}</strong></div>
    ...
    return `
      <div class="ps-bonus-shell">
        <div class="ps-bonus-hud-row">
          <div class="ps-bonus-mini-card"><label>Waktu</label><strong id="psBonusTimeValue">${Math.ceil(run.timeLeft || challenge.timeLimit || BONUS_RUN_DURATION)}</strong></div>
          <div class="ps-bonus-mini-card"><label>Skor Santuy</label><strong id="psBonusRunScore">${run.runScore || 0}</strong></div>
          <div class="ps-bonus-mini-card"><label>Item</label><strong id="psBonusCollectedValue">${run.collected || 0}</strong></div>
          <div class="ps-bonus-mini-card"><label>Kena Jebakan</label><strong id="psBonusHitValue">${run.hit || 0}</strong></div>
          <div class="ps-bonus-mini-card combo"><label>Double Jump</label><strong id="psBonusJumpValue">2/2</strong></div>
          <div class="ps-bonus-mini-card shield"><label>Invincible</label><strong id="psBonusShieldValue">-</strong></div>
        </div>
        <div class="ps-bonus-stage" id="psBonusStage">
          <div class="ps-bonus-sky-layer layer-1"></div><div class="ps-bonus-sky-layer layer-2"></div><div class="ps-bonus-sky-layer layer-3"></div>
          <div class="ps-bonus-rainbow"></div><div class="ps-bonus-cloud cloud-a"></div><div class="ps-bonus-cloud cloud-b"></div><div class="ps-bonus-cloud cloud-c"></div>
          <div class="ps-bonus-objects" id="psBonusObjects">${run.objects.map(renderBonusObject).join('')}</div>
          <div class="ps-bonus-track"></div>
          <button type="button" class="ps-bonus-runner" id="psBonusRunner" aria-label="PANJI pelari">
            <span class="ps-bonus-runner-glow"></span><span class="ps-bonus-runner-head"><span class="hat">PANJI</span><span class="eye left"></span><span class="eye right"></span><span class="mouth"></span></span><span class="ps-bonus-runner-body"><span>PBJ</span></span><span class="ps-bonus-runner-hand left"></span><span class="ps-bonus-runner-hand right"></span><span class="ps-bonus-runner-leg left"></span><span class="ps-bonus-runner-leg right"></span>
          </button>
          <div class="ps-bonus-control-pad">
            <button type="button" id="psBonusJumpBtn">Lompat</button>
            <button type="button" id="psBonusSlideBtn">Sliding</button>
          </div>
        </div>
        <div class="ps-bonus-bottom-panel">
          <div class="ps-bonus-status-card"><label>Narasi PANJI</label><strong id="psBonusStatusText">${escapeHtml(run.lastStatus || 'PANJI lari santuy...')}</strong><p>Bonus level ini sengaja ringan supaya pemain refresh dulu sebelum lanjut ke tantangan PBJ berikutnya.</p></div>
  ...
  function canGoNext() {
    const challenge = getCurrentChallenge();
    if (!challenge) return false;
    if (challenge.type === 'pipeline' || challenge.type === 'tenderRush' || challenge.type === 'bonusRun') return GAME_STATE.progress === 100;
    return GAME_STATE.answered;
  }
  ...
  function getDefaultLevelTime(challenge) {
    ...
    if (challenge.type === 'pipeline') return PIPELINE_LEVEL_TIME;
    return QUIZ_LEVEL_TIME;
  }

  function getHintMessage(challenge) {
    if (!challenge) return 'PANJI belum punya hint untuk soal ini.';
    if (challenge.type === 'bonusRun') return 'Hint PANJI: ini bonus santuy. Spasi/Tap untuk lompat, tekan lagi di udara untuk double jump, dan S atau panah bawah untuk sliding. Kalau kena perangkap, PANJI kebal beberapa detik dan item lewat tidak dihitung dulu.';
    if (challenge.type === 'tenderRush') return 'Tender Rush: baca paket dengan cepat lalu pilih jalur metode yang paling sesuai. Kamu juga bisa pakai tombol angka 1 sampai 5.';
    if (challenge.type === 'pipeline') return 'Susun dari tahap paling awal sampai paling akhir. Hindari kartu jebakan karena bisa menambah risiko.';
    return 'Baca kasusnya pelan-pelan, lalu pilih jawaban yang paling sesuai dengan konteks PBJ.';
  }
  ...
  function panjiForChallenge(challenge) {
    ...
    if (challenge.type === 'bonusRun') {
      showPanji('Bonus time! Otak kita sudah kerja keras. Sekarang bantu aku ngumpulin energi dulu. Spasi/Tap untuk lompat, double jump aktif, dan S / panah bawah untuk sliding.', 'happy');
      return;
    }
    if (challenge.type === 'pipeline') {
      showPanji('Ini soal pipeline. Susun kartu dari kiri ke kanan secara tertib. Aku akan jelasin setiap langkah benar supaya kamu paham, bukan cuma hafal.', 'thinking');
    } else if (challenge.type === 'tenderRush') {
      showPanji('Ini Tender Rush. Baca kartu paket yang turun, lalu tekan 1 untuk e-Katalog, 2 untuk Pengadaan Langsung, 3 untuk Tender/Seleksi, 4 untuk Swakelola, atau 5 untuk Dikecualikan.', 'thinking');
    } else {
      showPanji('Ini soal ABCD. Baca kata kuncinya pelan-pelan. Pilih jawaban yang paling sesuai prinsip dan tahapan PBJ, bukan yang sekadar paling cepat.', 'thinking');
    }
  }

  const __oldStartGame = startGame;
  startGame = function () {
    ensureBonusLevel8Inserted();
    clearBonusRunLoop();
    return __oldStartGame.apply(this, arguments);
  };

  const __oldLoadChallenge = loadChallenge;
  loadChallenge = function () {
    clearBonusRunLoop();
    const challengeIndex = GAME_STATE.order[GAME_STATE.index];
    const challenge = CHALLENGES[challengeIndex];
    if (!challenge || challenge.type !== 'bonusRun') {
      return __oldLoadChallenge.apply(this, arguments);
    }
    ...
    GAME_STATE.progress = 0;
    addLog('info', 'Bonus level dimulai', 'Bantu PANJI lari santai, kumpulkan item semangat, dan hindari jebakan lucu sampai waktu habis.');
    renderGame();
    panjiForChallenge(challenge);
  };

  const __oldBindGameEvents = bindGameEvents;
  bindGameEvents = function () {
    __oldBindGameEvents.apply(this, arguments);
    const startBtn = root && root.querySelector('#btnStartBonusRun');
    if (startBtn) startBtn.addEventListener('click', startBonusRun);
    const jumpBtn = root && root.querySelector('#psBonusJumpBtn');
    if (jumpBtn) jumpBtn.addEventListener('click', bonusJump);
    const slideBtn = root && root.querySelector('#psBonusSlideBtn');
    if (slideBtn) slideBtn.addEventListener('click', bonusSlide);
    const runner = root && root.querySelector('#psBonusRunner');
    if (runner) runner.addEventListener('click', bonusJump);
  };

  const __oldNextChallenge = nextChallenge;
  nextChallenge = function () {
    clearBonusRunLoop();
    return __oldNextChallenge.apply(this, arguments);
  };

  const __oldFinishGame = finishGame;
  finishGame = function () {
    clearBonusRunLoop();
    return __oldFinishGame.apply(this, arguments);
  };

  const __oldStopGameEarly = stopGameEarly;
  stopGameEarly = function () {
    clearBonusRunLoop();
    return __oldStopGameEarly.apply(this, arguments);
  };

  const __oldCalculateMaxScore = calculateMaxScore;
  calculateMaxScore = function () {
    return CHALLENGES.reduce((total, challenge) => {
      if (challenge.type === 'pipeline') return total + (challenge.idealIds.length * 10) + 20;
      if (challenge.type === 'tenderRush') return total + ((challenge.packages || []).length * 10) + 20;
      if (challenge.type === 'bonusRun') return total + 100;
      return total + 20;
    }, 0);
  };
