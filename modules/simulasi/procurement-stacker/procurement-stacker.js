...
const BONUS_RUN_CHALLENGE_RAW = {
  type: 'bonusRun',
  title: 'Soal 8 — PANJI Mood Booster',
  caseTitle: 'Bonus Santuy — Dunia Mimpi PANJI',
  desc: 'Bonus time! Otak kita sudah kerja keras. Sekarang bantu PANJI lari santai, ambil item semangat, dan hindari jebakan lucu untuk balikin mood.',
  budget: 'Bonus Santuy',
  difficulty: 'Level 8 — Bonus',
  timeLimit: 45,
  explanation: 'Bonus level ini dibuat sebagai jeda lucu dan penyegar suasana. Tidak ada game over, cukup bantu PANJI kumpulkan energi sampai waktunya habis.'
};
...
function ensureBonusLevelInserted() {
  const exists = CHALLENGES.some(ch => ch && ch.type === 'bonusRun');
  if (exists) return;
  const bonusChallenge = buildChallenge(BONUS_RUN_CHALLENGE_RAW);
  if (CHALLENGES.length >= 7) CHALLENGES.splice(7, 0, bonusChallenge);
  else CHALLENGES.push(bonusChallenge);
}
...
function createBonusRunState(challenge) {
  return {
    ...
    comboGlowUntil: 0,
    stumbleUntil: 0,
    lastStatus: 'Tekan Spasi atau tap untuk lompat. Bonus ini santuy, tidak ada game over.',
    counters: {
      star: 0, coin: 0, coffee: 0, mie: 0, doc: 0, gift: 0,
      sleep: 0, berkas: 0, deadline: 0, revisi: 0, loading: 0
    }
  };
}

function getCurrentBonusRunState() {
  const challenge = getCurrentChallenge();
...
function syncBonusHud() {
  ...
  const topProgress = root.querySelector('#psTopProgressValue');
  const topScore = root.querySelector('#psTopScoreValue');
  const topRisk = root.querySelector('#psTopRiskValue');
  const topWrong = root.querySelector('#psTopWrongValue');
  const caseScore = root.querySelector('#psCaseScoreValue');
  const caseRisk = root.querySelector('#psCaseRiskValue');
  const progressBar = root.querySelector('#psMainProgressBar');
  const levelTimeText = root.querySelector('#psLevelTimeText');
  const levelTimeBar = root.querySelector('#psLevelTimeBar');

  if (topProgress) topProgress.textContent = `${progress}%`;
  if (topScore) topScore.textContent = GAME_STATE.score;
  if (topRisk) topRisk.textContent = GAME_STATE.risk;
  if (topWrong) topWrong.textContent = GAME_STATE.wrong;
  if (caseScore) caseScore.textContent = GAME_STATE.score;
  if (caseRisk) caseRisk.textContent = GAME_STATE.risk;
  if (progressBar) progressBar.style.width = `${progress}%`;
  if (levelTimeText) levelTimeText.textContent = `${timeLeft}s`;
  if (levelTimeBar) levelTimeBar.style.width = `${Math.max(0, Math.min(100, (timeLeft / Math.max(1, Number(challenge.timeLimit || 45))) * 100))}%`;

  const map = {
    '#psBonusTimeValue': timeLeft,
    '#psBonusRunScore': run.runScore,
    '#psBonusCollectedValue': run.collectedCount,
    '#psBonusHitValue': run.hitCount,
    '#psBonusComboValue': `x${run.combo}`,
    '#psBonusStatusText': run.lastStatus
  };
  ...
  if (objectsEl) {
    objectsEl.innerHTML = run.objects.map(renderBonusRunObject).join('');
  }

  const runnerEl = root.querySelector('#psBonusRunner');
  if (runnerEl) {
    runnerEl.style.bottom = `${34 + Math.max(0, Number(run.runnerY || 0))}px`;
    runnerEl.classList.toggle('is-hit', performance.now() < Number(run.stumbleUntil || 0));
    runnerEl.classList.toggle('is-happy', performance.now() < Number(run.comboGlowUntil || 0));
  }

  const stageEl = root.querySelector('#psBonusStage');
  if (stageEl) {
    stageEl.classList.toggle('bonus-hit', performance.now() < Number(run.stumbleUntil || 0));
  }
}
...
function startBonusRun() {
  ...
  showPanji('Bonus time! Bantu aku ngumpulin energi. Ambil kopi, bintang, dan bonus poin. Hindari deadline, revisi dadakan, dan berkas numpuk ya.', 'happy');
  renderGame();
  syncBonusHud();
...
function pickRandomBonusObject(elapsed) {
  const hazardChance = elapsed > 28 ? 0.48 : elapsed > 16 ? 0.40 : 0.32;
...
function bonusRunCollectObject(obj) {
  ...
  run.combo += 1;
  run.comboGlowUntil = performance.now() + 280;
...
function bonusRunHitHazard(obj) {
  const run = getCurrentBonusRunState();
  if (!run || obj.hit) return;
  obj.hit = true;
  run.hitCount += 1;
...
function stepBonusRun(timestamp) {
  ...
  const speed = elapsed < 15 ? 300 : elapsed < 30 ? 350 : 395;
  run.nextSpawnAt -= dt;
  if (run.nextSpawnAt <= 0) {
    run.objects.push(pickRandomBonusObject(elapsed));
    const minGap = elapsed < 12 ? 0.95 : elapsed < 28 ? 0.78 : 0.62;
    const maxGap = elapsed < 12 ? 1.28 : elapsed < 28 ? 1.02 : 0.88;
...
function renderBonusRunChallenge(challenge) {
  const run = getCurrentBonusRunState() || createBonusRunState(challenge);
  ...
  return `
    <div class="ps-bonus-shell">
      ...
      <div class="ps-bonus-stage" id="psBonusStage">
        ...
        <div class="ps-bonus-sky-layer layer-3"></div>
        ...
        <button type="button" class="ps-bonus-runner" id="psBonusRunner" aria-label="PANJI pelari">
          ...
          <span class="ps-bonus-runner-head">
            <span class="hat">PANJI</span>
            <span class="eye left"></span>
            <span class="eye right"></span>
            <span class="mouth"></span>
          </span>
          <span class="ps-bonus-runner-body"><span>PBJ</span></span>
          <span class="ps-bonus-runner-hand left"></span>
          <span class="ps-bonus-runner-hand right"></span>
      ...
      <div class="ps-bonus-bottom-panel">
        <div class="ps-bonus-status-card">
          ...
          <p>Bonus level ini sengaja ringan supaya pemain refresh dulu sebelum lanjut ke tantangan PBJ berikutnya.</p>
        </div>
...
function canGoNext() {
  const challenge = getCurrentChallenge();
  if (!challenge) return false;
  if (challenge.type === 'pipeline' || challenge.type === 'tenderRush' || challenge.type === 'bonusRun') {
    return GAME_STATE.progress === 100;
  }
  return GAME_STATE.answered;
}
...
function getHintMessage(challenge) {
  ...
  if (challenge.type === 'bonusRun') {
    return 'Hint PANJI: bonus level ini santai. Cukup tekan Spasi atau tap untuk lompat. Prioritaskan kopi, mie, bintang, dan bonus poin. Kalau kena deadline atau berkas numpuk, skor memang turun sedikit, tapi permainan tetap lanjut sampai waktunya habis.';
  }
  if (challenge.type === 'tenderRush') {
    return 'Tender Rush: baca paket dengan cepat lalu pilih jalur metode yang paling sesuai. Kamu juga bisa pakai tombol angka 1 sampai 5.';
  }
  if (challenge.type === 'pipeline') {
    return 'Susun dari tahap paling awal sampai paling akhir. Hindari kartu jebakan karena bisa menambah risiko.';
  }
...
function panjiForChallenge(challenge) {
  if (!challenge) return;
  if (challenge.type === 'bonusRun') {
    showPanji('Bonus time! Otak kita sudah kerja keras. Sekarang bantu aku ngumpulin energi dulu ya. Tekan Spasi atau tap untuk lompat, ambil kopi dan bintang, lalu hindari deadline dan revisi dadakan.', 'happy');
    return;
  }
  if (challenge.type === 'tenderRush') {
    showPanji('Tender Rush aktif. Baca paket secepat mungkin, lalu pilih metode lewat tombol atau keyboard angka 1 sampai 5.', 'thinking');
    return;
  }
  if (challenge.type === 'pipeline') {
    showPanji('Susun tahapan dari awal ke akhir. Kalau ragu, klik Tanya PANJI untuk hint singkat.', 'happy');
    return;
  }
  showPanji('Baca kasusnya baik-baik, lalu pilih jawaban yang paling tepat.', 'thinking');
}
    ...
    <section class="ps-card">
      <div class="ps-card-head">
        <div>
          ...
          <p>Susun pipeline, jawab studi kasus, hadapi Tender Rush, lalu nikmati bonus level PANJI Mood Booster.</p>
        </div>
        <div class="ps-pill-row">
          <div class="ps-pill">${CHALLENGES.length} Level</div>
          <div class="ps-pill">Mixed Mode</div>
          <div class="ps-pill bonus">Bonus Level 8</div>
        </div>
      </div>

      <div class="ps-helper-note" style="margin-bottom:16px;">
        Soal akan bercampur: susun pipeline, pilihan ABCD, Tender Rush, dan bonus level PANJI Mood Booster.
        Tender Rush memakai tombol <b>1 sampai 5</b> untuk memilih metode pengadaan dengan cepat,
        sedangkan bonus level cukup <b>Spasi / Tap</b> untuk lompat.
      </div>

      ${renderPlayerForm()}

      <div class="ps-buttons" style="justify-content:flex-end; margin-top:18px;">
        <button type="button" class="ps-btn ps-btn-primary" id="btnStartGame">Mulai Main</button>
      </div>
    </section>
  `;
...
function startGame() {
  ...
  if (!panjiIntroAlreadyShown) {
    ...
    showPanji('Game dimulai ulang dari Soal 1. Aku langsung bantu kalau kamu butuh arahan.', 'thinking');
  }
...
function loadChallenge() {
  ...
  if (challenge.type === 'pipeline') {
    ...
    addLog('info', 'Challenge Tender Rush dimulai', 'Baca tutorial PANJI dulu, lalu pilih jalur metode dengan tombol 1 sampai 5.');
...
function renderGame() {
  ...
  const challenge = getCurrentChallenge();
  const isBonus = challenge.type === 'bonusRun';
  const bonusTimeLeft = isBonus ? Math.max(0, Math.ceil(Number((GAME_STATE.bonusRun && GAME_STATE.bonusRun.timeLeft) || challenge.timeLimit || 0))) : 0;
  const timerText = challenge.type === 'tenderRush' ? '-' : isBonus ? `${bonusTimeLeft}s` : `${GAME_STATE.levelTimeLeft || getDefaultLevelTime(challenge)}s`;
  const timerBar = challenge.type === 'tenderRush' ? 100 : isBonus ? Math.max(0, Math.min(100, (bonusTimeLeft / Math.max(1, Number(challenge.timeLimit || 45))) * 100)) : 100;
  ...
  root.innerHTML = `
    <section class="ps-card">
      <div class="ps-card-head">
        ...
        <div class="ps-pill-row">
          <div class="ps-pill ${challenge.type === 'pipeline' ? 'green' : challenge.type === 'tenderRush' ? 'rush' : challenge.type === 'bonusRun' ? 'bonus' : ''}">${getChallengeTypeLabel(challenge.type)}</div>
          <div class="ps-pill">Soal ${GAME_STATE.index + 1} / ${GAME_STATE.order.length}</div>
          ${GAME_STATE.selectedCardId ? '<div class="ps-pill warn">Kartu dipilih</div>' : ''}
          ${GAME_STATE.hintUsed ? '<div class="ps-pill warn">Hint PANJI dipakai</div>' : ''}
        </div>
      ...
      <div class="ps-case-panel">
        ...
        <div class="ps-case-box">
          <label>Skor</label>
          <strong id="psCaseScoreValue">${GAME_STATE.score}</strong>
        </div>
        <div class="ps-case-box">
          <label>Risiko</label>
          <strong id="psCaseRiskValue">${GAME_STATE.risk}</strong>
        </div>
      </div>

      <div class="ps-score-grid">
        <div class="ps-score-card"><label>Progress</label><strong id="psTopProgressValue">${GAME_STATE.progress}%</strong></div>
        <div class="ps-score-card"><label>Skor</label><strong id="psTopScoreValue">${GAME_STATE.score}</strong></div>
        <div class="ps-score-card"><label>Risiko</label><strong id="psTopRiskValue">${GAME_STATE.risk}</strong></div>
        <div class="ps-score-card"><label>Salah</label><strong id="psTopWrongValue">${GAME_STATE.wrong}</strong></div>
        <div class="ps-score-card ps-level-time-card">
          <label>${isBonus ? 'Waktu Bonus' : 'Waktu Level'}</label>
          <strong id="psLevelTimeText">${timerText}</strong>
          <div class="ps-mini-time-track"><div class="ps-mini-time-bar" id="psLevelTimeBar" style="width:${timerBar}%"></div></div>
        </div>
      </div>

      <div class="ps-progress-track"><div class="ps-progress-bar" id="psMainProgressBar" style="width:${GAME_STATE.progress}%"></div></div>

      <div class="ps-helper-row">
        <div class="ps-helper-note">
          <b>PANJI siap bantu.</b> Kalau klik <b>Tanya PANJI</b>, kamu dapat hint tetapi skor berkurang <b>${HINT_PENALTY}</b>.
        </div>
        <button type="button" class="ps-btn ps-btn-soft" id="btnPanjiHint">Tanya PANJI (-${HINT_PENALTY})</button>
      </div>
      ...
      <div class="ps-buttons">
        <button type="button" class="ps-btn ps-btn-soft" id="btnRestartGame">Mulai Ulang dari Soal 1</button>
        <button type="button" class="ps-btn ps-btn-primary" id="btnNextChallenge" ${canGoNext() ? '' : 'disabled'}>Lanjut Soal Berikutnya</button>
      </div>
    </section>
  `;
  ...
  const startBonusBtn = root.querySelector('#btnStartBonusRun');
  if (startBonusBtn) startBonusBtn.addEventListener('click', startBonusRun);

  const bonusTapArea = root.querySelector('#psBonusTapArea');
  if (bonusTapArea) bonusTapArea.addEventListener('click', handleBonusRunJump);

  const bonusRunner = root.querySelector('#psBonusRunner');
  if (bonusRunner) bonusRunner.addEventListener('click', handleBonusRunJump);

  requestAnimationFrame(updatePanjiAutoBottom);
}

const __originalNextChallenge = nextChallenge;
nextChallenge = function () {
  clearBonusRunLoop();
  return __originalNextChallenge.apply(this, arguments);
};

const __originalFinishGame = finishGame;
finishGame = function () {
  clearBonusRunLoop();
  return __originalFinishGame.apply(this, arguments);
};

const __originalStopGameEarly = stopGameEarly;
stopGameEarly = function () {
  clearBonusRunLoop();
  return __originalStopGameEarly.apply(this, arguments);
};
