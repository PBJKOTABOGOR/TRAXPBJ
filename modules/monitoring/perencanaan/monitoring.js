  ...
  window.__moduleInit = function ({ container }) {
    ...
    function getReadableHistoryKodeRup(rows, currentKodeRup) {
      ...
      const histories = (rows || []).map(item => {
        ...
        if (hist && /[;|,]/.test(hist)) {
          const separator = item.is_gabungan_rup || String(item.jenis_mapping || '').toLowerCase().includes('gabungan') ? ' + ' : ' → ';
    ...
    function groupRealisasi(realRows) {
      ...
      realRows.forEach(r => {
        ...
        */
        ...
        const jenisMappingRaw = String(r.jenis_mapping || '').toLowerCase();

        if (isGabunganRup || jenisMappingRaw.includes('gabungan')) {
          if (historyParts.length > 1) {
            historyLabel = historyParts.join(' + ');
          } else {
            historyLabel = getHistoryByScanningRow(r, kodeList[0]).replace(/ → /g, ' + ');
          }
        } else {
    ...
    function openDetailModal(kodeRup) {
      ...
      const historyDisplay = (historyVisible && historyVisible !== '-')
        ? historyVisible
