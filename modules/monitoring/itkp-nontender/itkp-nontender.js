...
window.__moduleInit = function ({ container }) {
  ...
  function openModal(title, subtitle, items, unitLabel = 'data', danger = false) {
    ...
    if (EL.modalCount) EL.modalCount.textContent = `${formatInt(items.length)} ${unitLabel}`;

    if (EL.modalList) {
      EL.modalList.innerHTML = items.length
        ? items.map(item => `<div class="modal-item${danger ? ' danger' : ''}">${escapeHtml(item)}</div>`).join('')
...
function getStatusLabel(status) {
  if (status === 'DOBEL_REALISASI') return 'Dobel Realisasi';
  if (status === 'SUDAH_REALISASI') return 'Sudah Realisasi';
  if (status === 'BELUM_REALISASI') return 'Belum Realisasi';
  return '-';
}
