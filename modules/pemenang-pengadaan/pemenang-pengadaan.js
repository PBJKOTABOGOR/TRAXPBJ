(function () {
  ...
  function makeRecord(row, type) {
    ...
    const record = {
      ...
      lpse: getField(row, ['lpse', 'nama lpse', 'nama_lpse', 'lpse penyelenggara', 'lpse_penyelenggara']) || '-',
  ...
  async function ensureData(state, progress) {
    if (state.datasetLoaded) return;

    const update = (percent, title, detail) => {
    ...
    if (cache.timestamp && now - cache.timestamp < cacheMaxAge && Array.isArray(cache.records)) {
      update(18, 'Membuka data cache...', 'Dataset terakhir masih hangat, portal tidak perlu menarik ulang semua sheet.');
      ...
      update(100, 'Data siap dipakai', 'Cache berhasil dimuat. Menampilkan hasil ke layar.');
      return;
    }

    update(8, 'Menyiapkan koneksi data...', 'Portal sedang membuka sumber tender dan non tender.');
    const tenderRows = await fetchSheetRows(PP_CONFIG.tenderSheet);
    update(38, 'Sheet tender berhasil dibaca', `Mengambil ${tenderRows.length.toLocaleString('id-ID')} baris dari ${PP_CONFIG.tenderSheet.title}.`);
    const nonTenderRows = await fetchSheetRows(PP_CONFIG.nonTenderSheet);
    update(68, 'Sheet non tender berhasil dibaca', `Mengambil ${nonTenderRows.length.toLocaleString('id-ID')} baris dari ${PP_CONFIG.nonTenderSheet.title}.`);

    const tenderRecords = tenderRows.map((row) => makeRecord(row, 'TENDER'));
    update(82, 'Merapikan struktur dataset...', 'Mengubah baris sheet menjadi record portal yang siap dicari dan difilter.');
    ...
    update(100, 'Data portal siap', `Total ${state.records.length.toLocaleString('id-ID')} paket berhasil dimuat dan siap ditampilkan.`);
  }
  ...
  window.__moduleInit = function ({ container }) {
    ...
    function renderLoading(options = {}) {
      ...
      const title = options.title || 'Menyiapkan portal...';
      const detail = options.detail || 'Portal sedang menarik data dan menyusun tampilan.';
      root.innerHTML = `
        <div class="pp-boot pp-boot--rich">
          <div class="pp-boot-bg"></div>
          <div class="pp-boot-panel">
            <div class="pp-boot-orb"></div>
            <div class="pp-boot-copy">
              <div class="pp-boot-kicker">Pemenang Pengadaan</div>
              <div class="pp-boot-title">${escapeHtml(title)}</div>
              <div class="pp-boot-text">${escapeHtml(detail)}</div>
              <div class="pp-boot-progress-row">
                <div class="pp-boot-track"><span style="width:${percent}%;"></span></div>
                <div class="pp-boot-percent">${Math.round(percent)}%</div>
              </div>
            </div>
          </div>
        </div>`;
    ...
    async function submitLogin(form) {
      ...
      renderLoading({ percent: 10, title: 'Menyiapkan portal...', detail: 'Menyusun halaman dan memeriksa sesi login.' });
      try {
    ...
    async function openPage(page) {
      ...
      if ((page === 'provider-search' || page === 'active-packages') && !state.datasetLoaded) {
        try {
          renderLoading({ percent: 18, title: 'Memeriksa akses login...', detail: 'Mencocokkan USERID dan PASSWORD ke sheet USERID yang Anda siapkan.' });
